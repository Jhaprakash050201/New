...
import certificationIcon from '@/assets/certificate.png'; // Import new icon
...
const services: Service[] = [
  ...
  },
  {
    ...
    description: 'Gain recognition for your expertise in AI by earning an industry-recognized certification. Whether you are mastering prompt engineering or AI tools, validate your skills and enhance your professional profile.',
    icon: certificationIcon, // Use new icon for certification
  },
];
...
const ServiceSection = () => {
  ...
  return (
    <div className="bg-[#D8E5F8] p-6 md:p-10 rounded-lg shadow-lg"> {/* Outer container */}
      ...
      <div className="mt-8 p-6 rounded-lg border border-white bg-transparent shadow-lg w-full md:w-[95%] lg:w-[95%] mx-auto"> 
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> {/* Arrange 3 cards per row */}
          {services.map((service, index) => (
            <motion.div
              key={service.title}
              className={`cursor-pointer p-6 rounded-lg shadow-md transition-transform transform duration-300 ease-in-out 
                hover:scale-105 hover:shadow-xl text-gray-800 bg-white`}
            ...
            >
              <div className="flex items-center mb-4">
                <img
                  ...
                  className="w-16 h-16 mr-4 transition-opacity duration-300"
                />
                <h3 className="text-xl font-semibold">{service.title}</h3>  {/* Increased font size for title */}
              </div>
              {expandedIndex === index && (
